 ...
 */
class SystemSQLPanel extends TWindow
{
    private $form;
    private $container;
    ...
    {
        parent::__construct();
        parent::setTitle(_t('SQL Panel'));
        parent::setSize(0.95, 0.95);
        ...
        $this->form->addFields( [ new TLabel(_t('Database'))], [ new TLabel(_t('Table'))] )->layout = ['col-sm-6', 'col-sm-6'];
        $this->form->addFields( [ $database], [ $table] )->layout = ['col-sm-6', 'col-sm-6'];
        $this->form->addFields( [ new TLabel('SELECT')] );
        $this->form->addFields( [$select] );
    ...
    {
        if (!empty($param['database']) && !empty($param['table']))
        {
            $obj = new stdClass;
    ...
    public function onGenerate($param)
    {
        try
        {
            ...
            $panel->setProperty('class', 'card panel noborder');
            $panel->add($datagrid);
    ...
    public static function onExportCSV($param)
    {
        try
        {
            ...
            (new TRequiredValidator)->validate(_t('Database'), $param['database']);
            (new TRequiredValidator)->validate(_t('Table'), $param['table']);
