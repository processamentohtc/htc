 ...
 */
class TLineChart extends TChartBase
{
    private $xlabels;
    private $ylabel;
    private $datasets;
    ...
    {
        $template = file_get_contents('lib/adianti/include/components/tlinechart/tlinechart.html');
        ...
        if (!empty($this->datasets))
        {
            ...
            {
                $datasets[] = ['label' => $name, 'data' => $values, 'backgroundColor' => $colors[$i], 'borderColor' => $colors[$i], 'pointRadius' => 5 ];
                $i++;
            }
        }
